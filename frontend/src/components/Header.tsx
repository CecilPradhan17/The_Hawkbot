...
// Exact SVG path for "Hawkbot" in Great Vibes cursive
// Source: danmarshall.github.io/google-font-to-svg-path
// Viewbox: 0 0 356 98
...
function HawkbotTitle({ animKey }: { animKey: number }) {
  const pathRef = useRef<SVGPathElement>(null)
  const [pathLength, setPathLength] = useState<number | null>(null)

  // Measure the actual path length after mount for a pixel-perfect dasharray
  useEffect(() => {
    if (pathRef.current) {
  ...
  return (
    <>
      {pathLength !== null && (
        <style>{`
          @keyframes hawkDraw_${animKey} {
            from { stroke-dashoffset: ${pathLength}; }
            to   { stroke-dashoffset: 0; }
          }
          @keyframes hawkFill_${animKey} {
            0%, 75% { fill: transparent; }
            100%    { fill: white; }
          }
          .hawk-animate-${animKey} {
            stroke-dasharray: ${pathLength};
            stroke-dashoffset: ${pathLength};
            animation:
              hawkDraw_${animKey} 3.5s cubic-bezier(0.4, 0, 0.2, 1) forwards,
              hawkFill_${animKey} 2s ease forwards;
          }
        `}</style>
      )}
      <svg
        ...
        viewBox="0 0 356 98"
        height="40"
        className="w-auto max-w-[160px] sm:max-w-[200px]"
      ...
      >
        <path
          ref={pathRef}
          d={HAWKBOT_PATH}
          // Show filled white immediately before measurement, then animate
          fill={pathLength === null ? 'white' : 'transparent'}
          stroke="white"
          strokeWidth="1.5"
          strokeLinecap="round"
          strokeLinejoin="round"
          fillRule="evenodd"
          className={pathLength !== null ? `hawk-animate-${animKey}` : ''}
        />
      </svg>
    </>
  )
